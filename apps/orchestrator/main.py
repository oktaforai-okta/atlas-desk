...
OKTA_DOMAIN = os.getenv("OKTA_DOMAIN", "example.oktapreview.com")
...
# default owner@example.test) so one login sees them all. Resolve once, cache,
...
def _assign_to_demo_user(jira, issue_key: str):
    ...
    email = os.getenv("JIRA_ASSIGNEE_EMAIL", "owner@example.test")
    if not email:
        return "", "disabled"
...
async def _run_demo(stream: EventStream, seed: int, inbound: Optional[dict] = None):
    ...
    iss = f"https://{OKTA_DOMAIN}/oauth2/ausEXAMPLEDelegate01"
...
async def _run_live(stream: EventStream, seed: int, inbound: Optional[dict] = None):
    ...
    ful_cas_issuer = f"https://{OKTA_DOMAIN}/oauth2/ausEXAMPLEWriteLan01"
