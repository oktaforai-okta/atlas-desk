...
| Orchestrator (FastAPI) | `apps/orchestrator` | **Render** (project `prj-EXAMPLEPROJECT`, Starter) |
...
1. In the shared Render project **prj-EXAMPLEPROJECT** → **New → Blueprint** (or Web Service), connect `oktaforai-okta/atlas-desk`, pick this deploy branch. Render reads `render.yaml` and proposes **atlas-orchestrator** (Starter, rootDir `apps/orchestrator`).
2. Set the secret env vars (marked `sync:false` in `render.yaml`, so Render prompts for them). Values live in the local `.secrets/` on the demo machine:
   ...
   | `INTAKE_PRIVATE_JWK` | paste the full JSON of `.secrets/wlpEXAMPLEAgentOne01.private.jwk.json` |
   | `DEVOPS_PRIVATE_JWK` | paste the full JSON of `.secrets/wlpEXAMPLEAgentTwo01.private.jwk.json` |
   | `JIRA_BASE_URL` | `.secrets/.env` (e.g. `https://your-site.atlassian.net`) |
...
- every case is assigned to **owner@example.test** — log into Jira as that account and you'll see them under "assigned to me", auto-resolved ones marked **Done**.
